...
Proof.
rewrite -[r in _ <= r < _]divq_num_den ler_pdivl_mulr ?ltr_pdivr_mulr ?ltr0z //.
by rewrite -rat1 -intrD -!intrM ler_int ltr_int lez_floor ?ltz_ceil.
Qed.

Lemma floorQ_epslt1 (r : rat) : 0 <= r - (floorQ r)%:Q < 1.
Proof. by rewrite subr_ge0 ltr_subl_addl floorQ_spec. Qed.

Lemma floorQ_grows (r1 r2 : rat) : r1 <= r2 -> floorQ r1 <= floorQ r2.
Proof.
move => Hr12.
suff: (floorQ r1)%:Q < (floorQ r2 + 1)%:Q by rewrite ltr_int ltz_addr1.
...
rewrite -[n in _ + n]rat1 -intrD !ltr_int !ltz_addr1 => ? ?.
by apply/eqP; rewrite eq_le; apply/andP.
Qed.

Lemma floorQ_int (m : int) : m = floorQ m%:Q.
Proof. by apply: floorQ_unique; rewrite ltr_int ltz_addr1 !lexx. Qed.
...
by apply/esym/floorQ_unique; rewrite ler_addl Heps1 intrD ltr_add2l.
Qed.
...
rewrite !ler_pdivl_mulr ?ltr_pdivr_mulr ?ltr0z // intrD -intrM ler_int.
move=> /andP[_ Hq] /andP[Hdiv /le_lt_trans ->] //.
rewrite -ltz_addr1 -(ltr_int [numDomainType of rat]) intrD.
...
rewrite -floorQ_int ler_pdivl_mulr ?ltr_pdivr_mulr ?ltr0z //.
